...
func (p DojangNext) Enter(l logrus.FieldLogger, c script.Context) bool {
	...
	if uint32(math.Floor(float64(c.MapId())/100))%100 >= 38 {
		...
		script.WarpById(l, c)(925020003, 0)
	...
	for i := int32(0); i < 5; i++ {
		...
		for _, cid := range cids {
			...
			character.WarpById(l)(c.WorldId(), c.ChannelId(), cid, restMapId, 0)
		}
	}
	return true
}
